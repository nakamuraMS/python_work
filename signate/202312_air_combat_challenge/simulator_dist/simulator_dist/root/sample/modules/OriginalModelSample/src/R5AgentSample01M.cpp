...
R5AgentSample01M::ActionInfo::ActionInfo(){
	dstDir<<1,0,0;
	dstAlt=10000.0;
	velRecovery=false;
	asThrottle=false;
...
R5AgentSample01M::TeamOrigin::TeamOrigin(){
	isEastSider=true;
	pos<<0,0,0;
}
R5AgentSample01M::TeamOrigin::TeamOrigin(bool isEastSider_,double dLine){
...
R5AgentSample01M::TeamOrigin::~TeamOrigin(){}
Eigen::Vector3d R5AgentSample01M::TeamOrigin::relBtoP(const Eigen::Vector3d& v) const{//陣営座標系⇛慣性座標系
	if(isEastSider){
...
Eigen::Vector3d R5AgentSample01M::TeamOrigin::relPtoB(const Eigen::Vector3d& v) const{//慣性座標系⇛陣営座標系
	if(isEastSider){
		return Eigen::Vector3d(-v(1),v(0),v(2));
	}else{
		return Eigen::Vector3d(v(1),-v(0),v(2));
...
R5AgentSample01M::R5AgentSample01M(const nl::json& modelConfig_,const nl::json& instanceConfig_)
...
R5AgentSample01M::~R5AgentSample01M(){}

void R5AgentSample01M::validate(){
	//Rulerに関する情報の取得
    ...
    for(auto&& e:parents){
		...
		if(parent->isinstance<CoordinatedFighter>()||parent->isinstance<SixDoFFighter>()){
			...
			throw std::runtime_error("R5AgentSample01M accepts only SixDoFFighter or CoordinatedFighter for its parent.");
		}
...
py::object R5AgentSample01M::observation_space(){
	return _observation_space;
}
py::object R5AgentSample01M::makeObs(){
	//observablesの収集
	extractObservables();
...
void R5AgentSample01M::extractObservables(){
	//味方機
	ourMotion.clear();
...
double R5AgentSample01M::calcDistanceMargin(const MotionState& myMotion, const nl::json& myObservables){
    //正規化した余剰燃料(距離換算)の計算
...
void R5AgentSample01M::makeImageObservation(){
...
Eigen::VectorXf R5AgentSample01M::makeVectorObservation(){
	Eigen::Vector3d pos,vel;
	double V,R;
...
py::object R5AgentSample01M::action_space(){
	return _action_space;
}

void R5AgentSample01M::deploy(py::object action_){
	last_action_obs=Eigen::MatrixXf::Zero(maxTrackNum["Friend"],last_action_dim);
...
py::object R5AgentSample01M::convertActionFromAnother(const nl::json& decision,const nl::json& command){
...
void R5AgentSample01M::controlWithAnotherAgent(const nl::json& decision,const nl::json& command){
...
int R5AgentSample01M::flatten_action(const Eigen::VectorXi& original_action,const Eigen::VectorXi& original_shape){
...
Eigen::VectorXi R5AgentSample01M::unflatten_action(const int& flattened_action,const Eigen::VectorXi& original_shape){
...
bool R5AgentSample01M::isInside(const int& lon, const int& lat){
    //ピクセル座標(lon,lat)が画像の範囲内かどうかを返す。
	return 0<=lon && lon<image_longitudinal_resolution && 0<=lat && lat<image_lateral_resolution;
}
Eigen::Vector2i R5AgentSample01M::rposToGrid(const Eigen::Vector3d& dr){
...
Eigen::Vector3d R5AgentSample01M::gridToRpos(const int& lon, const int& lat, const double& alt){
...
Eigen::Vector2i R5AgentSample01M::posToGrid(const Eigen::Vector3d& pos){
...
Eigen::Vector3d R5AgentSample01M::gridToPos(const int& lon, const int& lat, const double& alt){
...
void R5AgentSample01M::plotPoint(const Eigen::Vector3d& pos,const int& ch, const float& value){
...
void R5AgentSample01M::plotLine(const Eigen::Vector3d& pBegin,const Eigen::Vector3d& pEnd,const int& ch, const float& value){
...
double R5AgentSample01M::calcRHead(std::shared_ptr<PhysicalAssetAccessor> parent,const MotionState& myMotion,const Track3D& track){
...
double R5AgentSample01M::calcRTail(std::shared_ptr<PhysicalAssetAccessor> parent,const MotionState& myMotion,const Track3D& track){
	std::shared_ptr<FighterAccessor> f=std::dynamic_pointer_cast<FighterAccessor>(parent);
	Eigen::Vector3d rt=track.posI();
	Eigen::Vector3d vt=track.velI();
	Eigen::Vector3d rs=myMotion.pos;
...
double R5AgentSample01M::calcRNorm(std::shared_ptr<PhysicalAssetAccessor> parent,const MotionState& myMotion,const Track3D& track){
...
void exportR5AgentSample01M(py::module &m)
{
	...
	EXPOSE_LOCAL_CLASS(R5AgentSample01M)
	DEF_FUNC(R5AgentSample01M,validate)
	DEF_FUNC(R5AgentSample01M,observation_space)
	DEF_FUNC(R5AgentSample01M,makeObs)
	DEF_FUNC(R5AgentSample01M,flatten_action)
	DEF_FUNC(R5AgentSample01M,unflatten_action)
	DEF_FUNC(R5AgentSample01M,deploy)
	DEF_FUNC(R5AgentSample01M,control)
	DEF_FUNC(R5AgentSample01M,convertActionFromAnother)
	.def("convertActionFromAnother",[](R5AgentSample01M& v,const py::object& decision,const py::object& command){
